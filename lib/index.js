...
/**
 * @typedef {Object} Customer Address
...
export class Multipass {
	...
	encrypt(customerData) {
	    try {
	        ...
	        // Add authentication (HMAC) to ensure data integrity
	        const hmac = crypto.createHmac('sha256', this.hmacKey);
	        hmac.update(iv); // Include IV in the HMAC
	        hmac.update(encryptedData.slice(16)); // Exclude IV from encrypted data
	        const authTag = hmac.digest();
	
	        // Return the concatenated result: IV + Encrypted Data + Auth Tag
